...
template <class SS2, class GSOptions, class GSSelector>
                  ...
                  GSSelector* gs_selector)
{
  ...
  if (gs_options.colored_edge(ss2, eh))
  {
    graphics_scene.add_segment(eh->opposite()->vertex()->point(),
                               eh->vertex()->point(),
                               gs_options.edge_color(ss2, eh), gs_selector, eh);
  }
  else
  {
    graphics_scene.add_segment(eh->opposite()->vertex()->point(),
                               eh->vertex()->point(), gs_selector, eh);
  }
}
                    ...
                    GSSelector* gs_selector)
{
  ...
  {
    graphics_scene.add_point(vh->point(), gs_options.vertex_color(ss2, vh), gs_selector, vh);
  }
  else
                      ...
                      GSSelector* gs_selector)
{
  if (gs_options.are_edges_enabled())
  {
         ...
         it != ss2.halfedges_end(); ++it)
    {
      ...
      {
        compute_edge(ss2, it, graphics_scene, gs_options, gs_selector);
  ...
  if (gs_options.are_vertices_enabled())
  {
    ...
    {
      compute_vertex(ss2, it, graphics_scene, gs_options, gs_selector);
                           ...
                           CGAL::Graphics_scene& graphics_scene)
{
  ...
  CGAL::Graphics_scene_selector<CGAL_SS_TYPE,
    typename CGAL_SS_TYPE::Vertex_const_handle,
    typename CGAL_SS_TYPE::Halfedge_const_handle,
    typename CGAL_SS_TYPE::Face_const_handle,
    void> gs_selector(false);

  add_to_graphics_scene(ass2, graphics_scene, drawingFunctor, &gs_selector);
}
                           ...
                           const GSOptions& gs_options)
{
  ...
  add_to_graphics_scene(ass2, graphics_scene, gs_options, &gs_selector);
}
...
// Specialization of draw function.
template <class K, class GSSelector>
void draw(const CGAL_SS_TYPE &ass2, GSSelector *gs_selector,
          const char *title="Straight Skeleton Basic Viewer")
{
  CGAL::Graphics_scene buffer;
  add_to_graphics_scene(ass2, buffer, gs_selector);
  draw_graphics_scene(buffer, title);
}
...
{
  CGAL::Graphics_scene buffer;
  add_to_graphics_scene(ass2, buffer, gs_options, gs_selector);
  draw_graphics_scene(buffer, title);
}
